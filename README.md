...
## LLM и конфигурация
- Используется OpenRouter: один API ключ дает доступ к нескольким моделям, можно настроить приоритеты по нодам графа.
...
- Дефолтный набор (бесплатные модели OpenRouter): интенты/тональность - `gpt-oss-20b:free` с fallback на `mistral-7b` и `llama-3.2-3b`; суммаризация/поиск - `gpt-oss-20b:free` / `mistral-7b`; финальный ответ - `gpt-oss-20b:free` (первый приоритет) с fallback на `gpt-oss-120b:free`, `mistral-7b`, `moonshotai/kimi-k2`, `llama-3.2-3b` (см. `llm_config.yaml`).

## Запуск
1) Установите зависимости: `pip install -r requirements.txt`  
2) Скопируйте `.env.example` в `.env`, укажите `OPENROUTER_API_KEY` (и при необходимости `OPENROUTER_REFERER`, `OPENROUTER_TITLE`), отредактируйте `llm_config.yaml`, если нужен другой набор моделей/параметров.  
...
## Соответствие ТЗ (AI-agent_task.docx)
...
- `OPENROUTER_REFERER` — реферер для OpenRouter (рекомендуется указать домен или репозиторий).  
- `OPENROUTER_TITLE` — произвольное название приложения, попадает в заголовок `X-Title`.  
